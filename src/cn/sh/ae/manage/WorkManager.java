				...
				try {
					...
					if (ss.getSmsstatus() != null
						...
						for (Atm atm : atmList) {
							...
							if (!type.equals("1")) {
								sendType = "2";
								if (atm.getAllstatus().equals("钞箱") && type.equals("2"))
									typeStr = "缺钞";
...
//									sendType, level);
							...
							if ((level.equals("2") || level.equals("3")) && atm.getAllstatus().equals("钞箱") && !type.equals("7"))
								continue;
							else
							// 判断改用户是否存在且是否被注销
								...
								if (smsUser != null && smsUser.getStatus() != null
									...
									if (levelFlag) {
										...
										if (sendPhone != null
											...
											if ((level.equals("2") || level.equals("3")) && atm.getAllstatus().equals("钞箱") && !type.equals("7"))
											{
												logger.info("钞箱预警，未缺钞，出列");
												continue;
											}
											if (level.equals("2") && atm.getAllstatus().equals("钞箱"))
											{
												typeStr = "缺钞已过一小时";
												logger.info("钞箱："+type+level);
												
											}
											else if (level.equals("3") && atm.getAllstatus().equals("钞箱"))
											{
												
												typeStr = "缺钞已过二小时";
												logger.info("钞箱："+type+level);
											}
											else if (atm.getAllstatus().equals("流水打印") && level.equals("2"))
											{
												typeStr = "缺纸已过一小时";
											}
											else if (atm.getAllstatus().equals("流水打印") && level.equals("3"))
											{
												typeStr = "缺纸已过二小时";
											}
											// 组装短信
